...
def exercise07(sd,m,s):
    ...
    random_numbers = np.random.default_rng().normal(m, sd, s)
...
def exercise11(n):
    ...
    array_reshaped = np.reshape(array_1d, (3, n//3))
...
def exercise13(n):
    ...
    for x in a:
        ...
        i = y[count]
        count += 1
    s = pd.Series(data = y, index = dates)
    s.plot(kind = "line")
...
def exercise14(words):
    ...
    df = pd.DataFrame({"Words" : words, "Length" : [len(x) for x in words]})
